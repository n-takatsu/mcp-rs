# WordPress Security Plugin対応チェックリスト

## ✅ 必須確認事項

### 1. Application Password設定
```json
...
### 3. WP Site Guard設定の影響確認

#### ✅ 影響しない（MCP動作に問題なし）
- ログインページURL変更（wp-admin → wp-login123等）
- 管理画面へのIP制限
- ログイン試行回数制限
- 画像認証/2段階認証
...
1. **定期的な接続テスト**
   - MCPサーバー起動時の自動テスト実装済み
   
2. **Application Passwordの定期更新**
   - 3-6ヶ月毎の更新推奨
