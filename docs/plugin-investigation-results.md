# WP Site GuardとWP Maintenance Mode除外設定の詳細調査結果
...
### 誤解の訂正
1. **WP Site Guardに有料版は存在しない**
2. **WP Maintenance Modeは無料版で除外設定可能**
3. **除外設定だけでは不十分な場合が多い**

### 推奨アクション
1. **即座**：wp-config.php修正でREST API許可
2. **短期**：WP Maintenance Mode再インストール＋適切設定
3. **長期**：MCPサーバー側メンテナンス検出機能活用
...
1. **WP Site Guard設定確認**（現在インストール済み）
2. **wp-config.php一時修正**（即効性）
3. **WP Maintenance Mode再検討**（根本解決）
4. **MCPサーバー接続テスト**（動作確認）