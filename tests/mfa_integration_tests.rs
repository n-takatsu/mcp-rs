...
async fn test_complete_mfa_flow() {
    ...
    async fn test_session_mfa_with_multiple_devices() {
        ...
        for (ua, ip, name) in &devices {
            ...
            device_trust.trust_device(user_id, &fp, ua, ip, name).await.unwrap();
        }
    ...
    async fn test_device_max_limit_enforcement() {
        ...
        for i in 0..3 {
            let fp = DeviceTrustManager::generate_fingerprint(&format!("ua{}", i), &format!("ip{}", i), None);
            manager
                .trust_device(user_id, &fp, &format!("ua{}", i), &format!("ip{}", i), &format!("Device {}", i))
                .await
                .unwrap();
        }
    ...
    async fn test_concurrent_session_management() {
        ...
        for i in 0..10 {
            ...
            let handle = tokio::spawn(async move {
                ...
                manager
                    .verify_session_mfa(&session_id, &uid, &challenge.challenge_id, MfaMethod::Totp, None)
                    .await
                    .unwrap();
    ...
    async fn test_complete_user_journey() {
        ...
        let qr_code = totp_secret.to_qr_code("TestApp", "user@example.com").unwrap();
        ...
        session_manager
            .verify_session_mfa("session_001", "user123", &challenge.challenge_id, MfaMethod::Totp, None)
            .await
            .unwrap();
        ...
        device_trust
            .trust_device("user123", &fingerprint, "user_agent", "192.168.1.1", "My Laptop")
            .await
            .unwrap();
        ...
        let session_state = session_manager.get_session_state("session_002").await.unwrap();
