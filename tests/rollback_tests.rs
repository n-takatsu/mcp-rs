...
async fn test_snapshot_creation() {
    ...
    let snapshot_id = rollback_manager.create_snapshot(
        ...
        SnapshotCreationReason::Manual { created_by: "test".to_string() },
    ).await.unwrap();
...
async fn test_auto_rollback_trigger() {
    ...
    // 事前にスナップショットを作成
    let _snapshot_id = rollback_manager.create_snapshot(
        policy,
        None,
        Default::default(),
        DeploymentState::Idle,
...
async fn test_rollback_config_update() {
    ...
    rollback_manager.update_config(new_config.clone()).await.unwrap();
...
async fn test_event_subscription() {
    ...
    if let Some(RollbackEvent::SnapshotCreated { snapshot_id: received_id, .. }) = events.first() {
...
async fn test_snapshot_cleanup() {
    ...
    for i in 0..5 {
        let _snapshot_id = rollback_manager.create_snapshot(
            ...
            Default::default(),
            DeploymentState::Idle,
            SnapshotCreationReason::Manual { created_by: format!("test_{}", i) },
        ).await.unwrap();
    }
...
async fn test_metrics_collection() {
    ...
    let _rollback_id = rollback_manager.initiate_manual_rollback(
        snapshot_id,
        "test_user".to_string(),
        "Test rollback".to_string(),
    ).await.unwrap();
    ...
    // 自動ロールバックもテスト
    let _snapshot_id2 = rollback_manager.create_snapshot(
        policy,
        None,
        Default::default(),
        DeploymentState::Idle,
    ...
    let _auto_rollback_id = rollback_manager.trigger_auto_rollback(
...
async fn test_should_trigger_auto_rollback() {
    ...
    assert!(!rollback_manager.should_trigger_auto_rollback(&normal_metrics).await.unwrap());
    
    // 高エラー率
    let high_error_metrics = create_test_metrics(8.0, 200.0);
    assert!(rollback_manager.should_trigger_auto_rollback(&high_error_metrics).await.unwrap());
    
    // 高応答時間
    let slow_response_metrics = create_test_metrics(2.0, 1200.0); // 1200ms > 1000ms 閾値
    assert!(rollback_manager.should_trigger_auto_rollback(&slow_response_metrics).await.unwrap());
    
    // 両方の問題
    let problematic_metrics = create_test_metrics(8.0, 1200.0);
    assert!(rollback_manager.should_trigger_auto_rollback(&problematic_metrics).await.unwrap());
}
