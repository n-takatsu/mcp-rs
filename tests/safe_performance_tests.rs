...
use mcp_rs::handlers::database::safety::{SafetyManager, LoopGuard, SafetyError};
...
impl SafePerformanceTest {
    ...
    pub async fn safe_basic_query_test(&self) -> Result<TestResult, SafetyError> {
        ...
        let test_result = timeout(test_timeout, async {
            ...
            for i in 0..max_iterations {
                ...
                let query_result = self.safety_manager.safe_execute(
                    || async {
            ...
            Ok(TestResult {
                ...
                average_duration: Duration::from_nanos(
                    results.iter().map(|r| r.duration.as_nanos() as u64).sum::<u64>() / max_iterations
                ),
                results,
            })
        }).await;
    ...
    pub async fn safe_concurrent_test(&self) -> Result<TestResult, SafetyError> {
        ...
        let test_result = timeout(test_timeout, async {
            ...
            for task_id in 0..concurrent_tasks {
                let safety_manager_ref = &self.safety_manager; // 参照を使用
                let handle = tokio::spawn(async move {
                    ...
                    for query_id in 0..queries_per_task {
                        ...
                        let start = Instant::now();
                        let query_result = safety_manager_ref.safe_execute(
                            || async {
                                tokio::time::sleep(Duration::from_millis(50)).await;
                                Ok::<String, Box<dyn std::error::Error + Send + Sync>>(
                                    format!("Task {} Query {} result", task_id, query_id)
                                )
                            },
            ...
            for handle in task_handles {
                match handle.await {
                    ...
                    Err(e) => return Err(SafetyError::OperationFailed(
            ...
            let avg_duration = if !all_results.is_empty() {
                Duration::from_nanos(
                    all_results.iter().map(|r| r.duration.as_nanos() as u64).sum::<u64>() / total_count
                )
            } else {
                Duration::default()
            };
    ...
    pub async fn safe_error_handling_test(&self) -> Result<TestResult, SafetyError> {
        let error_scenarios = 3; // 本来の4から削減
        ...
        let test_result = timeout(test_timeout, async {
            ...
            for (scenario_idx, scenario) in scenarios.iter().enumerate() {
                ...
                for iteration in 0..iterations_per_scenario {
                    ...
                    // 意図的にエラーを発生させるテスト
                    let error_result = self.safety_manager.safe_execute(
                        || async {
                            // モックエラー（実際のテストではDB操作）
                    ...
                    all_results.push(QueryTestResult {
                        iteration: (iteration as u64 + scenario_idx as u64 * iterations_per_scenario as u64),
                        duration,
