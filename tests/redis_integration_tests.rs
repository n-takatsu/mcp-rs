...
use mcp_rs::handlers::database::{
    ...
    types::{DatabaseConfig, ConnectionConfig, PoolConfig, DatabaseType, SecurityConfig, FeatureConfig},
};
...
async fn test_redis_basic_set_get() {
    ...
    let result = connection
        .execute(
            "INSERT INTO test_key VALUES ('test_value')",
            &[]
        )
        .await;
...
async fn test_redis_delete() {
    ...
    // DELETE the key
    let result = connection
        .execute("DELETE FROM test_delete_key", &[])
        .await;
    assert!(result.is_ok());
...
async fn test_redis_transaction_commit() {
    ...
    let _ = tx.execute(
        "INSERT INTO tx_test_key VALUES ('tx_value')",
        &[]
    ).await;
...
async fn test_redis_transaction_info() {
    ...
    assert_eq!(tx_info.isolation_level, mcp_rs::handlers::database::engine::IsolationLevel::Serializable);
