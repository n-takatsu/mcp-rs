...
impl PostgresTransaction {
    ...
    pub async fn create_savepoint(&mut self, name: &str) -> Result<(), DatabaseError> {
        ...
        if let Some(tx) = tx_guard.as_mut() {
            sqlx::query(&format!("SAVEPOINT {}", name))
                ...
                .await
                .map_err(|e| {
                    DatabaseError::TransactionFailed(format!("Failed to create savepoint: {}", e))
                })?;
    ...
    pub async fn release_savepoint(&mut self, name: &str) -> Result<(), DatabaseError> {
        ...
        if let Some(tx) = tx_guard.as_mut() {
            sqlx::query(&format!("RELEASE SAVEPOINT {}", name))
                ...
                .await
                .map_err(|e| {
                    DatabaseError::TransactionFailed(format!("Failed to release savepoint: {}", e))
                })?;
    ...
    pub async fn set_isolation_level(
        ...
        if let Some(tx) = tx_guard.as_mut() {
            ...
            sqlx::query(sql).execute(&mut **tx).await.map_err(|e| {
                DatabaseError::TransactionFailed(format!("Failed to set isolation level: {}", e))
            })?;
    ...
    async fn query_internal(
        ...
        if let Some(tx) = tx_guard.as_mut() {
            ...
            let columns: Vec<crate::handlers::database::types::ColumnInfo> =
    ...
    async fn commit(mut self: Box<Self>) -> Result<(), DatabaseError> {
        ...
        if let Some(tx) = tx_guard.take() {
            tx.commit().await.map_err(|e| {
                DatabaseError::TransactionFailed(format!("Failed to commit: {}", e))
            })?;
    ...
    async fn rollback(mut self: Box<Self>) -> Result<(), DatabaseError> {
        ...
        if let Some(tx) = tx_guard.take() {
            tx.rollback().await.map_err(|e| {
                DatabaseError::TransactionFailed(format!("Failed to rollback: {}", e))
            })?;
    ...
    fn transaction_info(&self) -> crate::handlers::database::engine::TransactionInfo {
        crate::handlers::database::engine::TransactionInfo {
            ...
            started_at: chrono::Utc::now()
                - chrono::Duration::milliseconds(self.duration_ms() as i64),
