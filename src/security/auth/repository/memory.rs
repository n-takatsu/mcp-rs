...
use crate::security::auth::types::{AuthUser, AuthError, PasswordHasher};
...
impl UserRepository for InMemoryUserRepository {
    ...
    async fn verify_password(&self, email: &str, password: &str) -> Result<Option<AuthUser>, AuthError> {
        ...
        if let Some(stored) = users.values().find(|u| u.user.email.as_deref() == Some(email)) {
    ...
    async fn get_password_hash(&self, user_id: &str) -> Result<Option<String>, AuthError> {
        let users = self.users.read().await;
        Ok(users.get(user_id).and_then(|stored| stored.password_hash.clone()))
    }
...
mod tests {
    ...
    async fn test_update_password_hash() {
        ...
        repo.create_user(&user, Some(old_hash.to_string())).await.unwrap();
        repo.update_password_hash("user1", new_hash.to_string()).await.unwrap();
