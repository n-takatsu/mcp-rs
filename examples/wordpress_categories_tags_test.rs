...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    match handler.create_category("MCP Test Category", Some("Category created by MCP-RS"), None).await {
        Ok(category) => {
            ...
            if let Some(category_id) = category.id {
                ...
                match handler.update_category(category_id, Some("MCP Updated Category"), Some("Updated description")).await {
                    Ok(updated_category) => {
    ...
    // Create a test tag
    println!("\n🏷️ Creating test tag...");
    match handler.create_tag("mcp-test", Some("Tag created by MCP-RS")).await {
        Ok(tag) => {
            ...
            if let Some(tag_id) = tag.id {
                ...
                match handler.update_tag(tag_id, Some("mcp-updated"), Some("Updated tag description")).await {
                    Ok(updated_tag) => {
