...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    println!("✓ Secret generated: {}", secret.encoded());
    println!();
    ...
    match verifier.verify(&secret, &current_code) {
        ...
        Ok(false) => println!("✗ Code verification failed"),
    ...
    match verifier.verify(&secret, "000000") {
        Ok(true) => println!("✗ Invalid code accepted (should not happen!)"),
        Ok(false) => println!("✓ Invalid code correctly rejected"),
        Err(e) => println!("✗ Verification error: {}", e),
    }
    println!();
...
#[cfg(not(feature = "mfa"))]
fn main() {
    println!("このデモを実行するには、mfaフィーチャーを有効にしてください。");
