...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    println!("[OK] Current TOTP code: {}", current_code);
    println!();

    // Verify the code
    println!("--- Step 5: Verify Code ---");
    match verifier.verify(&secret, &current_code) {
        Ok(true) => println!("[OK] Code verified successfully!"),
        Ok(false) => println!("[FAIL] Code verification failed"),
    ...
    match verifier.verify(&secret, "000000") {
        Ok(true) => println!("[FAIL] Invalid code accepted (should not happen!)"),
        Ok(false) => println!("[OK] Invalid code correctly rejected"),
        Err(e) => println!("[ERROR] Verification error: {}", e),
    }
    println!();
    ...
    println!("[OK] 8-digit code: {}", eight_digit_code);
...
#[cfg(not(feature = "mfa"))]
fn main() {
    println!("This demo requires the mfa feature to be enabled.");
    println!("Run with: cargo run --example mfa_totp_demo --features mfa");
}
