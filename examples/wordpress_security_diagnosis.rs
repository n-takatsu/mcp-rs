...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    println!("🔐 パスワード: {}", if password.is_some() { "設定済み" } else { "未設定" });
    println!();
    ...
    match client.get(&wordpress_url).send().await {
        Ok(response) => {
            ...
            if let Some(security) = headers.get("x-frame-options") {
                println!("   🔒 セキュリティヘッダー検出: X-Frame-Options = {:?}", security);
            }
        }
        Err(e) => {
            println!("   ❌ サイト接続失敗: {}", e);
    ...
    let test_agents = [
        ("Standard", "mcp-rs/1.0"),
        ("Browser-like", "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36"),
    ...
    if let (Some(user), Some(pass)) = (&username, &password) {
        ...
        println!("   🔐 Basic認証ヘッダー: Authorization: Basic [{}文字]", auth_b64.len());
