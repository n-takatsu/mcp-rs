...
use mcp_rs::rollback::{
    RollbackManager, RollbackConfig, SnapshotCreationReason, 
    RollbackEvent, MetricsSnapshot, PolicyMetrics, SystemMetrics
};
...
async fn demo_snapshot_creation(
    ...
    let snapshot_reasons = vec![
        ...
        SnapshotCreationReason::Manual { created_by: "admin".to_string() },
        SnapshotCreationReason::AnomalyDetected { reason: "High error rate detected".to_string() },
    ];

    for (i, reason) in snapshot_reasons.into_iter().enumerate() {
        let snapshot_id = rollback_manager.create_snapshot(
            stable_policy.clone(),
            None,
            Default::default(),
        ...
        println!("   ✅ Created snapshot {}: {} ({:?})", i + 1, snapshot_id, reason);
...
async fn demo_manual_rollback(
    ...
    let snapshot_id = rollback_manager.create_snapshot(
        ...
        Default::default(),
        DeploymentState::Idle,
        SnapshotCreationReason::Manual { created_by: "demo".to_string() },
    ).await?;
    
    println!("   📸 Created target snapshot: {}", snapshot_id);
    
    // 手動ロールバックを開始
    let rollback_id = rollback_manager.initiate_manual_rollback(
        snapshot_id,
        "demo_user".to_string(),
...
async fn demo_auto_rollback(
    ...
    println!("      - Canary error rate: {:.2}% (threshold: 3.0%)", problematic_metrics.canary_metrics.error_rate);
    println!("      - Canary response time: {:.2}ms", problematic_metrics.canary_metrics.avg_response_time_ms);
    
    // 自動ロールバックをトリガー
    let rollback_id = rollback_manager.trigger_auto_rollback(
...
async fn demo_staged_rollback(
    ...
    for (stage_name, target_percentage) in stages {
        println!("   🔄 Stage '{}': Targeting {}% canary traffic", stage_name, target_percentage);
...
async fn handle_rollback_event(event: RollbackEvent) {
    match event {
        RollbackEvent::AutoRollbackTriggered { rollback_id, reason, .. } => {
            info!("🚨 Auto rollback triggered: {} (Reason: {})", rollback_id, reason);
        },
        RollbackEvent::ManualRollbackInitiated { rollback_id, initiated_by, reason, .. } => {
            info!("🔧 Manual rollback initiated by {}: {} (Reason: {})", initiated_by, rollback_id, reason);
        },
        RollbackEvent::RollbackProgress { rollback_id, stage_name, progress_percentage, .. } => {
            info!("🔄 Rollback progress: {} - {} ({}%)", rollback_id, stage_name, progress_percentage);
        },
        RollbackEvent::RollbackCompleted { rollback_id, duration_ms, .. } => {
            info!("✅ Rollback completed: {} (Duration: {}ms)", rollback_id, duration_ms);
        },
        RollbackEvent::RollbackFailed { rollback_id, error_message, .. } => {
            error!("❌ Rollback failed: {} (Error: {})", rollback_id, error_message);
        },
        RollbackEvent::SnapshotCreated { snapshot_id, creation_reason, .. } => {
            info!("📸 Snapshot created: {} (Reason: {:?})", snapshot_id, creation_reason);
        },
        RollbackEvent::AnomalyDetected { anomaly_type, severity, recommended_action, .. } => {
            warn!("⚠️  Anomaly detected: {} (Severity: {:?}) - {}", anomaly_type, severity, recommended_action);
        },
    }
}
